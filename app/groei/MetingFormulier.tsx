...
export function MetingFormulier({ showForm }: Props) {
  ...
  async function opslaan() {
    ...
    setTimeout(() => {
      setSuccess(false);
      setOpen(false);
      setGewicht(""); setLengte(""); setHoofd(""); setNota("");
      router.refresh();
    }, 1500);
  }
  ...
  return (
    <div className="bg-[var(--rho-cream)]/8 border border-[var(--rho-cream)]/15 rounded-2xl p-5 space-y-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        <div>
          <label className="block text-[var(--rho-cream)]/60 text-xs font-body mb-1.5">Gewicht (kg)</label>
          <input type="text" inputMode="decimal" placeholder="4,25" value={gewicht} onChange={(e) => setGewicht(e.target.value)} className={inputClass} />
        </div>
        <div>
          <label className="block text-[var(--rho-cream)]/60 text-xs font-body mb-1.5">Lengte (cm)</label>
          <input type="text" inputMode="decimal" placeholder="54,5" value={lengte} onChange={(e) => setLengte(e.target.value)} className={inputClass} />
        </div>
        <div>
          <label className="block text-[var(--rho-cream)]/60 text-xs font-body mb-1.5">Hoofd (cm)</label>
          <input type="text" inputMode="decimal" placeholder="36,0" value={hoofd} onChange={(e) => setHoofd(e.target.value)} className={inputClass} />
        </div>
      </div>
      <div>
        <label className="block text-[var(--rho-cream)]/60 text-xs font-body mb-1.5">Nota (optioneel)</label>
        <textarea placeholder="Bijv. 6-weekse consultatie" value={nota} onChange={(e) => setNota(e.target.value)} rows={2} className={`${inputClass} resize-none`} />
      </div>
      <div className="flex gap-3">
        <Button variant="ghost" className="flex-1" onClick={() => setOpen(false)} disabled={loading}>Annuleer</Button>
        <Button variant="primary" className="flex-1" loading={loading} onClick={opslaan} disabled={!gewicht && !lengte && !hoofd}>
